...
internal static class StringDiffer
{
    ...
    public static string PrintFirstDifference(string expected, string actual)
    {
        ...
        using StringReader expectedReader = new(expected);
        using StringReader actualReader = new(actual);
        ...
        while (expectedLine is not null || actualLine is not null)
        {
            ...
            stringBuilder.AppendLine(CultureInfo.InvariantCulture, $"----------------------------- Expected: Around Line {line} -----------------------------");
            if (previousExpectedLine is not null)
            ...
            if (nextExpectedLine is not null)
                stringBuilder.AppendLine(nextExpectedLine);

            stringBuilder.AppendLine(CultureInfo.InvariantCulture, $"----------------------------- Actual: Around Line {line} -----------------------------");
            if (previousActualLine is not null)
            ...
            else if (actualLine[^1] is ' ' or '\t')
            {
                var lastNonWhitespace = FindIndexOfNonWhitespace(actualLine);
                stringBuilder.AppendLine(actualLine[..lastNonWhitespace] + MakeWhiteSpaceVisible(actualLine[lastNonWhitespace..]));
            }
            else
            {
    ...
    private static string? MakeWhiteSpaceVisible(string? value) => value?.Replace(' ', '·').Replace('\t', '→');
}