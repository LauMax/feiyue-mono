...
{
    public override string ToString() => DocSerializer.Serialize(this);
    ...
    public static readonly HardLine HardLineIfNoPreviousLineSkipBreakIfFirstInGroup = new(true, true);
    ...
    public static LeadingComment LeadingComment(string comment, CommentType commentType) => new() { Type = commentType, Comment = comment };

    public static TrailingComment TrailingComment(string comment, CommentType commentType) => new() { Type = commentType, Comment = comment };

    public static Doc Concat(List<Doc> contents) => contents.Count == 1 ? contents[0] : new Concat(contents);
    ...
    public static ForceFlat ForceFlat(List<Doc> contents) => new() { Contents = contents.Count == 0 ? contents[0] : Concat(contents) };

    public static ForceFlat ForceFlat(params Doc[] contents) => new() { Contents = contents.Length == 0 ? contents[0] : Concat(contents) };

    public static Group Group(List<Doc> contents) => new() { Contents = contents.Count == 1 ? contents[0] : Concat(contents) };
    ...
    public static Doc IndentIf(bool condition, Doc contents) => condition ? Indent(contents) : contents;
    ...
    public static IndentIfBreak IndentIfBreak(Doc contents, string groupId) => new(contents, groupId);
    ...
    public static AlwaysFits AlwaysFits(Doc printedTrivia) => new(printedTrivia);
