...
internal sealed class SharedFunc<T> : SemaphoreSlim
{
    ...
    private static readonly ConcurrentDictionary<string, SharedFunc<T>> RunningFuncs = [];

    private SharedFunc()
        : base(1, int.MaxValue) { }

    public static async Task<T> GetOrAddAsync(string key, Func<Task<T>> factory, CancellationToken cancellationToken)
    {
        var workspace = RunningFuncs.GetOrAdd(key, _ => new());
        await workspace.WaitAsync(cancellationToken);
