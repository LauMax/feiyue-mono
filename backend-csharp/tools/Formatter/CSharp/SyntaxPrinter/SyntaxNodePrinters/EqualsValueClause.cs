...
internal static class EqualsValueClause
{
    public static Doc Print(EqualsValueClauseSyntax node, PrintingContext context)
    {
        ...
        if (node is { Parent: PropertyDeclarationSyntax, Value: not AnonymousObjectCreationExpressionSyntax })
        {
            // keeping line
        }
        ...
        Doc result = Doc.Group(" ", Token.PrintWithSuffix(node.EqualsToken, separator, context), Node.Print(node.Value, context));
