...
internal static class BinaryExpression
{
    public static Doc Print(BinaryExpressionSyntax node, PrintingContext context)
    {
        ...
        return shouldNotIndent ? Doc.Group(docs) : Doc.Group(docs[0], Doc.Indent(docs.Skip(1).ToList()));
    }
    ...
    private static List<Doc> PrintBinaryExpression(SyntaxNode node, PrintingContext context)
    {
        ...
        try
        {
            ...
            var shouldGroup =
                (binaryExpressionSyntax.Kind() != SyntaxKind.CoalesceExpression || ShouldGroupNullCoalescingSyntax(binaryExpressionSyntax))
                && binaryExpressionSyntax.Kind() != binaryExpressionSyntax.Parent!.Kind()
                && GetPrecedence(binaryExpressionSyntax) != GetPrecedence(binaryExpressionSyntax.Parent!)
            ...
            if (binaryOnTheRight)
                docs.Add(Node.Print(binaryExpressionSyntax.Left, context), Doc.Line, Token.Print(binaryExpressionSyntax.OperatorToken, context), " ");

            var possibleBinary = binaryOnTheRight ? binaryExpressionSyntax.Right : binaryExpressionSyntax.Left;
            ...
            // print them normally.
            if (possibleBinary is BinaryExpressionSyntax childBinary && ShouldFlatten(binaryExpressionSyntax.OperatorToken, childBinary.OperatorToken))
            ...
            var right = Doc.Concat(Doc.Line, Token.Print(binaryExpressionSyntax.OperatorToken, context), " ", Node.Print(binaryExpressionSyntax.Right, context));
    ...
    private static bool ShouldGroupNullCoalescingSyntax(BinaryExpressionSyntax binaryExpressionSyntax)
    {
        ...
        static bool IsTerminalInvocation(ExpressionSyntax expression) =>
            expression switch
            {
                InvocationExpressionSyntax invocation => !HasInvocationExcludingLeftmostParenthesized(invocation.Expression),
                ConditionalAccessExpressionSyntax conditionalAccess => IsTerminalInvocationInWhenNotNull(conditionalAccess.WhenNotNull)
                    && !HasInvocationExcludingLeftmostParenthesized(conditionalAccess.Expression),
                AwaitExpressionSyntax awaitExpression => IsTerminalInvocation(awaitExpression.Expression),
                _ => false
            };
        ...
        static bool IsTerminalInvocationInWhenNotNull(ExpressionSyntax whenNotNull)
        {
            ...
            return !whenNotNull.DescendantNodesAndSelf().OfType<InvocationExpressionSyntax>().Any(invocation => invocation != rightmost);
        }
        ...
        static ExpressionSyntax GetRightmostExpression(ExpressionSyntax expression) =>
            expression switch
            {
                ...
                ConditionalAccessExpressionSyntax conditional => GetRightmostExpression(conditional.WhenNotNull),
                MemberAccessExpressionSyntax memberAccess => GetRightmostExpression(memberAccess.Expression),
                _ => expression
            };
        ...
        static bool HasInvocationExcludingLeftmostParenthesized(ExpressionSyntax expression)
        {
            ...
            return expression
                ...
                .Any(node =>
                    (leftmost is not ParenthesizedExpressionSyntax excludedNode || !IsDescendantOrSelf(node, excludedNode))
        ...
        static ExpressionSyntax GetLeftmostExpression(ExpressionSyntax expression) =>
            expression switch
            {
                MemberAccessExpressionSyntax memberAccess => GetLeftmostExpression(memberAccess.Expression),
                InvocationExpressionSyntax invocation => GetLeftmostExpression(invocation.Expression),
                ConditionalAccessExpressionSyntax conditional => GetLeftmostExpression(conditional.Expression),
                AwaitExpressionSyntax awaitExpr => GetLeftmostExpression(awaitExpr.Expression),
                _ => expression
            };

        static bool IsDescendantOrSelf(SyntaxNode node, SyntaxNode ancestor) => node == ancestor || node.Ancestors().Contains(ancestor);
    }

    private static bool ShouldFlatten(SyntaxToken parentToken, SyntaxToken nodeToken) => GetPrecedence(parentToken) == GetPrecedence(nodeToken);
    ...
    private static int GetPrecedence(SyntaxToken syntaxToken) =>
        syntaxToken.RawSyntaxKind() switch
        {
            ...
            _ => throw new($"No precedence defined for {syntaxToken}")
        };
}