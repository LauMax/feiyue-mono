...
internal static class InterpolatedStringExpression
{
    ...
    public static Doc Print(InterpolatedStringExpressionSyntax node, PrintingContext context)
    {
        ...
        if (node.Contents.Any(o => o is InterpolationSyntax && o.ToString().Contains('\n', StringComparison.Ordinal)))
            return Doc.Concat(Token.PrintLeadingTrivia(node.GetLeadingTrivia(), context), node.ToString());

        if (node.StringStartToken.RawSyntaxKind() == SyntaxKind.InterpolatedMultiLineRawStringStartToken)
            return RawString(node, context);

        List<Doc> docs = [Token.PrintWithoutLeadingTrivia(node.StringStartToken, context)];
        ...
        return Doc.Concat(Token.PrintLeadingTrivia(node.StringStartToken, context), Doc.ForceFlat(docs), Token.PrintTrailingTrivia(node.StringEndToken));
    }

    private static Doc RawString(InterpolatedStringExpressionSyntax node, PrintingContext context)
    {
        var endDelimiterIsIndented =
            node.StringEndToken.Text.Replace("\r", string.Empty, StringComparison.Ordinal).Replace("\n", string.Empty, StringComparison.Ordinal)[0] is '\t' or ' ';

        List<Doc> contents = [Token.Print(node.StringStartToken, context), endDelimiterIsIndented ? Doc.HardLineNoTrim : Doc.LiteralLine];
        foreach (var content in node.Contents)
        {
            ...
            {
                if (string.IsNullOrEmpty(textSyntax.TextToken.ValueText))
                    continue;

                var lines = textSyntax.TextToken.ValueText.Split(LineSeparators, StringSplitOptions.None);
