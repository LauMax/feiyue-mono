    ...
    public static Doc Print(CollectionExpressionSyntax node, PrintingContext context)
    {
        Doc separator = node.Parent
            is ArgumentSyntax { NameColon: null }
                ...
                or AssignmentExpressionSyntax { Parent: not (ObjectCreationExpressionSyntax or InitializerExpressionSyntax or ExpressionStatementSyntax) }
                or EqualsValueClauseSyntax { Parent: not VariableDeclaratorSyntax }
            ? Doc.Null
            : Doc.IfBreak(Doc.Line, Doc.Null);

        var alwaysBreak = node.Elements.FirstOrDefault() is ExpressionElementSyntax { Expression: CollectionExpressionSyntax };
        ...
        var result = Doc.Concat(
            ...
            node.Elements.Any()
                ? Doc.Indent(
                    ...
                    SeparatedSyntaxList.PrintWithTrailingComma(node.Elements, Node.Print, alwaysBreak ? Doc.HardLine : Doc.Line, context, node.CloseBracketToken))
                : Doc.Null,
            node.Elements.Any()
                ? alwaysBreak
                    ? Doc.HardLine
            ...
            node.CloseBracketToken.LeadingTrivia.Any(o => o.IsComment() || o.IsDirective)
                ? Doc.Concat(Doc.Indent(Token.PrintLeadingTrivia(node.CloseBracketToken, context)), Doc.HardLine)
                : Doc.Null,
