...
internal static class SwitchExpression
{
    public static Doc Print(SwitchExpressionSyntax node, PrintingContext context)
    {
        var sections = Doc.Group(
            Doc.Indent(Doc.HardLine, SeparatedSyntaxList.PrintWithTrailingComma(node.Arms, PrintArm, Doc.HardLine, context, node.CloseBraceToken)),
            Doc.HardLine);
    ...
    private static Doc PrintArm(SwitchExpressionArmSyntax switchExpressionArm, PrintingContext context)
    {
        var arrowHasComment = switchExpressionArm.EqualsGreaterThanToken.LeadingTrivia.Any(o => o.IsComment());

        var groupId2 = arrowHasComment ? string.Empty : Guid.NewGuid().ToString();
        var innerContents = arrowHasComment
            ? Doc.Indent(Doc.Concat(Doc.Line, Token.PrintWithSuffix(switchExpressionArm.EqualsGreaterThanToken, " ", context), Node.Print(switchExpressionArm.Expression, context)))
            : Doc.Concat(
                " ",
        ...
        return Doc.Concat(
            ...
            Token.PrintLeadingTrivia(switchExpressionArm.Pattern.GetLeadingTrivia(), context.WithSkipNextLeadingTrivia()),
            Doc.Group(
                Doc.GroupWithId(
                    groupId1,
                    Doc.Concat(
                        Node.Print(switchExpressionArm.Pattern, context),
                        switchExpressionArm.WhenClause is not null ? Node.Print(switchExpressionArm.WhenClause, context) : Doc.Null)),
                innerContents));
    }
}