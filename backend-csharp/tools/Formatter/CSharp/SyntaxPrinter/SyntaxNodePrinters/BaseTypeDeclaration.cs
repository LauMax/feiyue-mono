    ...
    public static Doc Print(BaseTypeDeclarationSyntax node, PrintingContext context)
    {
        ...
        if (node is TypeDeclarationSyntax typeDeclarationSyntax)
        {
            ...
            if (typeDeclarationSyntax.Members.Count > 0)
                members = () => Doc.Indent(MembersWithForcedLines.Print(typeDeclarationSyntax, typeDeclarationSyntax.Members, context));
        ...
        else if (node is EnumDeclarationSyntax enumDeclarationSyntax)
        {
            if (enumDeclarationSyntax.Members.Count > 0)
                members = () => Doc.Indent(MembersWithForcedLines.Print(enumDeclarationSyntax, enumDeclarationSyntax.Members, context));
            ...
            var baseListDoc = Doc.Concat(
                ...
                node.BaseList.Types.Count > 1
                    ? Doc.Indent(
                        ...
                        Doc.Line,
                        SeparatedSyntaxList.Print(node.BaseList.Types, Node.Print, Doc.Line, context, startingIndex: 1))
                    : Doc.Null);
        ...
        if (members is not null)
        {
            ...
            docs.Add(Doc.HardLine, Token.Print(node.OpenBraceToken, context), membersContent, Doc.HardLine, Token.Print(node.CloseBraceToken, context));
        }
        else if (node.OpenBraceToken.RawSyntaxKind() != SyntaxKind.None)
        {
            Doc separator = node.CloseBraceToken.LeadingTrivia.Any(o => o.RawSyntaxKind() is not (SyntaxKind.WhitespaceTrivia or SyntaxKind.EndOfLineTrivia)) ? Doc.Line : " ";

            docs.Add(separator, Token.Print(node.OpenBraceToken, context), separator, Token.Print(node.CloseBraceToken, context));
        }
