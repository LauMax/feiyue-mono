...
internal static partial class BaseMethodDeclaration
{
    ...
    private static readonly Regex RemoveWhiteSpaceLineEndingsRegex = RemoveWhiteSpaceLineEndingsGenerator();
#else
    ...
    public static Doc Print(CSharpSyntaxNode node, PrintingContext context)
    {
        ...
        if (node is BaseMethodDeclarationSyntax baseMethodDeclarationSyntax)
        {
            ...
            {
                identifier = () => Doc.Concat(Token.Print(destructorDeclarationSyntax.TildeToken, context), Token.Print(destructorDeclarationSyntax.Identifier, context));
            }
        ...
        if (attributeLists is { Count: > 0 })
        {
            ...
            {
                var attributeStart = attributeLists.Value[0].GetLeadingTrivia().First().GetLocation().SourceSpan.Start;

                var methodWithoutAttributes = node.GetText().Replace(0, syntaxTriviaList.First().GetLocation().SourceSpan.Start - attributeStart, string.Empty).ToString().Trim();

                docs.Add(RemoveWhiteSpaceLineEndingsRegex.Replace(methodWithoutAttributes, context.Options.LineEnding));
            }
        ...
        if (modifiers is { Count: > 0 })
        {
            ...
            declarationGroup.Add(Modifiers.PrintSorterWithoutLeadingTrivia(modifiers.Value, context));
        }
        ...
        if (explicitInterfaceSpecifier is not null)
            declarationGroup.Add(Node.Print(explicitInterfaceSpecifier.Name, context), Token.Print(explicitInterfaceSpecifier.DotToken, context));
        ...
        if (node is ConversionOperatorDeclarationSyntax conversionOperatorDeclarationSyntax)
        {
            declarationGroup.Add(
                Token.PrintWithSuffix(conversionOperatorDeclarationSyntax.ImplicitOrExplicitKeyword, " ", context),
                Token.PrintWithSuffix(conversionOperatorDeclarationSyntax.OperatorKeyword, " ", context),
                Token.PrintWithSuffix(conversionOperatorDeclarationSyntax.CheckedKeyword, " ", context),
        ...
        else if (node is OperatorDeclarationSyntax operatorDeclarationSyntax)
        {
            declarationGroup.Add(
                ...
                " ",
                operatorDeclarationSyntax.ExplicitInterfaceSpecifier is not null ? Node.Print(operatorDeclarationSyntax.ExplicitInterfaceSpecifier, context) : Doc.Null,
        ...
        if (typeParameterList?.Parameters.Any() == true)
        ...
        if (parameterList is not null)
        {
            if (parameterList.Parameters.Any())
                ...
                declarationGroup.Add(Token.Print(parameterList.OpenParenToken, context), Token.Print(parameterList.CloseParenToken, context));
        ...
        if (constructorInitializer is not null)
        {
            ...
            declarationGroup.Add(
                Doc.Group(Doc.Indent(Doc.HardLine), Doc.Indent(colonToken), Token.Print(constructorInitializer.ThisOrBaseKeyword, context), Doc.Indent(argumentList)));
        }

        docs.Add(Doc.Group(declarationGroup));

        if (constraintClauses is not null)
