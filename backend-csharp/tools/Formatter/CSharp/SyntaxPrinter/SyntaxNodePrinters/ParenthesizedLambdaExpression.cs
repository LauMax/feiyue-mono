...
{
    public static Doc Print(ParenthesizedLambdaExpressionSyntax node, PrintingContext context) => Doc.Concat(PrintHead(node, context), PrintBody(node, context));
    ...
    public static Doc PrintHead(ParenthesizedLambdaExpressionSyntax node, PrintingContext context) =>
        Doc.Concat(
            ...
            node.ReturnType is not null ? Doc.Concat(Node.Print(node.ReturnType, context), " ") : Doc.Null,
    ...
    public static Doc PrintBody(ParenthesizedLambdaExpressionSyntax node, PrintingContext context)
    {
        if (node.Body is BlockSyntax block)
            return Doc.Concat(block.Statements.Count > 0 ? Doc.HardLine : " ", Block.Print(block, context));

        var body = Node.Print(node.Body, context);

        if (node.ParameterList.Parameters.Count == 0 && node.Parent?.Parent is ArgumentListSyntax { Arguments.Count: 1 })
