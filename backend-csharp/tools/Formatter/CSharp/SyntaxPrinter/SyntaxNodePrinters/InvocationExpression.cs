...
internal sealed record PrintedNode(CSharpSyntaxNode Node, Doc Doc);
...
{
    public static Doc Print(InvocationExpressionSyntax node, PrintingContext context) => PrintMemberChain(node, context);
    ...
    public static Doc PrintMemberChain(ExpressionSyntax node, PrintingContext context)
    {
        ...
        var groups = printedNodes.Any(o => o.Node is InvocationExpressionSyntax) ? GroupPrintedNodesPrettierStyle(printedNodes) : GroupPrintedNodesOnLines(printedNodes);
        ...
        var forceOneLine =
            (
                ...
                && (
                    groups
                        ...
                        .Any(o =>
                            o[^1].Node is not (InvocationExpressionSyntax or ElementAccessExpressionSyntax or PostfixUnaryExpressionSyntax { Operand: InvocationExpressionSyntax }))
                    // if the last group contains just a !, make sure it doesn't end up on a new line
                    || (groups[^1].Count == 1 && groups[^1][0].Node is PostfixUnaryExpressionSyntax)))
            ...
            || groups[0]
                .Any(o =>
                    o.Node
                        is LiteralExpressionSyntax { Token.RawKind: (int)SyntaxKind.MultiLineRawStringLiteralToken }
                            or InterpolatedStringExpressionSyntax { StringStartToken.RawKind: (int)SyntaxKind.InterpolatedMultiLineRawStringStartToken }
                    || (
                        o.Node is LiteralExpressionSyntax { Token.Text.Length: > 0 } literalExpressionSyntax
                        && literalExpressionSyntax.Token.Text.Contains('\n', StringComparison.Ordinal)));

        if (forceOneLine)
            return Doc.Group(oneLine);

        var shouldMerge = shouldMergeFirstTwoGroups;
        var expanded = Doc.Concat(
            Doc.Concat(groups[0].Select(o => o.Doc).ToArray()),
            shouldMerge
                ? Doc.IndentIf(groups.Count > 2 && groups[1].Last().Doc is not Group { Contents: IndentDoc }, Doc.Concat(groups[1].Select(o => o.Doc).ToArray()))
                : Doc.Null,
            PrintIndentedGroup(groups.Skip(shouldMerge ? 2 : 1).ToList()));

        return

                oneLine.Skip(1).Any(DocUtilities.ContainsBreak)
                || groups[0].Any(o => o.Node is ArrayCreationExpressionSyntax or ObjectCreationExpressionSyntax { Initializer: not null })
                || groups[0].First().Node is ParenthesizedExpressionSyntax { Expression: SwitchExpressionSyntax or QueryExpressionSyntax }
                || (parent is ExpressionStatementSyntax expressionStatementSyntax && expressionStatementSyntax.SemicolonToken.LeadingTrivia.Any(o => o.IsComment()))
                || groups.Count == 1
                ? expanded
                : Doc.ConditionalGroup(Doc.Concat(oneLine), expanded);
    }

    private static void FlattenAndPrintNodes(ExpressionSyntax expression, List<PrintedNode> printedNodes, PrintingContext context)
    {
        /*
        ...
        */
        if (expression is InvocationExpressionSyntax invocationExpressionSyntax)
        {
            ...
            printedNodes.Add(new PrintedNode(invocationExpressionSyntax, ArgumentList.Print(invocationExpressionSyntax.ArgumentList, context)));
        }
        else if (expression is ElementAccessExpressionSyntax elementAccessExpression)
        {
            ...
            printedNodes.Add(new PrintedNode(elementAccessExpression, Node.Print(elementAccessExpression.ArgumentList, context)));
        }
        else if (expression is MemberAccessExpressionSyntax memberAccessExpressionSyntax)
        {
            ...
            printedNodes.Add(
                new PrintedNode(
                    ...
                    Doc.Concat(Token.Print(memberAccessExpressionSyntax.OperatorToken, context), Node.Print(memberAccessExpressionSyntax.Name, context))));
        }
        else if (expression is ConditionalAccessExpressionSyntax conditionalAccessExpressionSyntax)
        {
            ...
            printedNodes.Add(new PrintedNode(conditionalAccessExpressionSyntax, Token.Print(conditionalAccessExpressionSyntax.OperatorToken, context)));
            FlattenAndPrintNodes(conditionalAccessExpressionSyntax.WhenNotNull, printedNodes, context);
        }
        else if (expression is PostfixUnaryExpressionSyntax { Operand: InvocationExpressionSyntax or MemberAccessExpressionSyntax } postfixUnaryExpression)
        {
            FlattenAndPrintNodes(postfixUnaryExpression.Operand, printedNodes, context);
            printedNodes.Add(new PrintedNode(postfixUnaryExpression, Token.Print(postfixUnaryExpression.OperatorToken, context)));
        }
        else
        {
    ...
    private static List<List<PrintedNode>> GroupPrintedNodesOnLines(List<PrintedNode> printedNodes)
    {
        ...
        List<PrintedNode> currentGroup = [printedNodes[0]];
        ...
        for (var index = 1; index < printedNodes.Count; index++)
        {
            if (printedNodes[index].Node is ConditionalAccessExpressionSyntax)
            {
                ...
                printedNodes[index].Node is MemberAccessExpressionSyntax or MemberBindingExpressionSyntax or IdentifierNameSyntax
    ...
    private static List<List<PrintedNode>> GroupPrintedNodesPrettierStyle(List<PrintedNode> printedNodes)
    {
        // We want to group the printed nodes in the following manner
        //
        //   a().b.c().d().e
        ...
        if (
            ...
            && printedNodes[index].Node is ElementAccessExpressionSyntax or PostfixUnaryExpressionSyntax)
        {
            currentGroup.Add(printedNodes[index]);
            index++;
        }
        ...
        for (; index < printedNodes.Count; index++)
        {
            if (hasSeenNodeThatRequiresBreak && printedNodes[index].Node is MemberAccessExpressionSyntax or ConditionalAccessExpressionSyntax)
            {
                groups.Add(currentGroup);
                currentGroup = [];
            ...
            if (printedNodes[index].Node is InvocationExpressionSyntax or ElementAccessExpressionSyntax)
    ...
    private static Doc PrintIndentedGroup(List<List<PrintedNode>> groups)
    {
        ...
        DocListBuilder result = new(groups.Count * 2);
        ...
        for (int index = 0; index < groups.Count; index++)
        {
            Doc GetPossibleContents()
            {
                ...
                if (nextGroup[0].Node is not MemberAccessExpressionSyntax { Name: IdentifierNameSyntax { Identifier.Text: "ThenInclude" } })
                    return Doc.Null;

                index++;
                return Doc.Indent(Doc.HardLine, Doc.Group(nextGroup.Select(p => p.Doc).ToArray()), GetPossibleContents());
            }
        ...
        var output = Doc.Indent(Doc.Group(result.ToArray()));
        result.Dispose();

        return output;
    }

    private static bool ShouldMergeFirstTwoGroups(List<List<PrintedNode>> groups, SyntaxNode? parent)
    {
        ...
        // Merge short identifiers like Job, this, base with their first member access
        if (firstNode is not (IdentifierNameSyntax { Identifier.Text.Length: <= 4 } or ThisExpressionSyntax or PredefinedTypeSyntax or BaseExpressionSyntax))
            return false;

        return groups[1].Count == 1
            || parent is SimpleLambdaExpressionSyntax or ArgumentSyntax or BinaryExpressionSyntax or ExpressionStatementSyntax
            || groups[1].Skip(1).First().Node is InvocationExpressionSyntax or ElementAccessExpressionSyntax or PostfixUnaryExpressionSyntax;
    }
}