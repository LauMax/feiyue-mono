...
internal static class Block
{
    ...
    {
        if (node.Statements.Count == 0 && node.Parent is MethodDeclarationSyntax && !Token.HasComments(node.CloseBraceToken))
            return Doc.Concat(" ", Token.Print(node.OpenBraceToken, context), " ", Token.Print(node.CloseBraceToken, context));

        Doc statementSeparator = node.Parent is AccessorDeclarationSyntax && node.Statements.Count <= 1 ? Doc.Line : Doc.HardLine;
        ...
        var result = Doc.Group(
            node.Parent is ParenthesizedLambdaExpressionSyntax or BlockSyntax or GlobalStatementSyntax ? Doc.Null : Doc.Line,
