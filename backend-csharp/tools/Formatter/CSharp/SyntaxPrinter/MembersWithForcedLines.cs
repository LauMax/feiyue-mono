...
{
    [SkipLocalsInit]
    public static List<Doc> Print<T>(CSharpSyntaxNode node, IReadOnlyList<T> members, PrintingContext context, bool skipFirstHardLine = false)
        where T : MemberDeclarationSyntax
    {
        List<Doc> result = new(members.Count * 3);
        ...
        for (var memberIndex = 0; memberIndex < members.Count; memberIndex++)
        {
            ...
            if (
                ...
                && classDeclaration.Modifiers.Any(o => o.RawSyntaxKind() is SyntaxKind.AbstractKeyword)
                && methodDeclaration.Modifiers.Any(o => o.RawSyntaxKind() is SyntaxKind.AbstractKeyword))
            {
            ...
            var leadingTrivia = member.GetLeadingTrivia().Select(o => o.RawSyntaxKind()).ToImmutableHashSet();
            ...
            foreach (var syntaxTrivia in leadingTrivia)
            {
                ...
                else if (syntaxTrivia is SyntaxKind.EndIfDirectiveTrivia or SyntaxKind.EndRegionDirectiveTrivia)
                {
            ...
            if (printExtraNewLines)
                result.Add(ExtraNewLines.Print(member));
            else if (addBlankLine && !triviaContainsEndIfOrRegion && !skipAddingLineBecauseIgnoreEnded)
                result.Add(Doc.HardLine);
            ...
            if (
                addBlankLine
                && (
                    (!triviaContainsEndIfOrRegion && leadingTrivia.Contains(SyntaxKind.IfDirectiveTrivia) && !leadingTrivia.Contains(SyntaxKind.EndOfLineTrivia))
                    || (
                        ...
                        && (!leadingTrivia.Contains(SyntaxKind.EndOfLineTrivia) || leadingTrivia.Contains(SyntaxKind.SingleLineCommentTrivia))
                        && !printExtraNewLines)))
            {
