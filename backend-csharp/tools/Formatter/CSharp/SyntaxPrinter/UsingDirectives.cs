...
internal static class UsingDirectives
{
    ...
    public static Doc PrintWithSorting(SyntaxList<UsingDirectiveSyntax> usings, PrintingContext context, bool printExtraLines)
    {
        if (usings.Count == 0)
            return Doc.Null;
        ...
        List<SyntaxTrivia> initialComments = [];
        List<SyntaxTrivia> triviaWithinIf = [];
        var foundIfDirective = false;
        var keepUsingsUntilEndIf = false;
        foreach (var leadingTrivia in usings[0].GetLeadingTrivia())
        {
            if (leadingTrivia.RawSyntaxKind() == SyntaxKind.DisabledTextTrivia && leadingTrivia.ToFullString().TrimStart().StartsWith("extern alias", StringComparison.Ordinal))
            {
            ...
            if (leadingTrivia.RawSyntaxKind() is SyntaxKind.DefineDirectiveTrivia or SyntaxKind.UndefDirectiveTrivia)
            {
        ...
        if (keepUsingsUntilEndIf)
        {
            while (usingList.Count != 0)
            {
                ...
                if (firstUsing.GetLeadingTrivia().Any(o => o.RawSyntaxKind() == SyntaxKind.EndIfDirectiveTrivia))
                    break;
            }
        }
        ...
        if (
            reorderedDirectives
            && (
                usings.Any(o => o.ToFullString().Contains("#endif", StringComparison.Ordinal))
                || (
                    usings[0].Parent is NamespaceDeclarationSyntax namespaceDeclarationSyntax
                    && namespaceDeclarationSyntax.GetLeadingTrivia().ToFullString().Contains("#if", StringComparison.Ordinal))))
        {
    ...
    private static IEnumerable<List<UsingData>> GroupUsings(List<UsingDirectiveSyntax> usings, SyntaxTriviaList triviaOnFirstUsing, PrintingContext context)
    {
        List<UsingData> globalSystemUsings = [];
        List<UsingData> globalUsings = [];
        List<UsingData> globalAliasUsings = [];
        List<UsingData> systemUsings = [];
        List<UsingData> aliasNameUsings = [];
        List<UsingData> regularUsings = [];
        List<UsingData> staticSystemUsings = [];
        List<UsingData> staticUsings = [];
        List<UsingData> aliasUsings = [];
        List<UsingData> directiveGroup = [];
        var ifCount = 0;
        var isFirst = true;
        ...
        foreach (var usingDirective in usings)
        {
            ...
            if (ifCount > 0)
            {
                directiveGroup.Add(new UsingData { Using = usingDirective, LeadingTrivia = PrintLeadingTrivia(usingDirective) });
            }
            else
            {
                if (openIf)
                    directiveGroup.Add(new UsingData { LeadingTrivia = PrintLeadingTrivia(usingDirective) });

                UsingData usingData = new() { Using = usingDirective, LeadingTrivia = !openIf ? PrintLeadingTrivia(usingDirective) : Doc.Null };
        ...
        {
            var result = isFirst ? Token.PrintLeadingTrivia(triviaOnFirstUsing, context) : Token.PrintLeadingTrivia(value.GetLeadingTrivia(), context);
    ...
    private sealed class UsingData
    {
    ...
    private sealed class DefaultOrder : IComparer<UsingDirectiveSyntax?>
    {
        public int Compare(UsingDirectiveSyntax? x, UsingDirectiveSyntax? y)
        {
            ...
            if (x?.Alias is not null && y?.Alias is not null)
                return string.Compare(x.Alias.ToFullString().Trim(), y.Alias.ToFullString().Trim(), StringComparison.OrdinalIgnoreCase);

            return string.Compare(x?.Name?.ToFullString().Trim(), y?.Name?.ToFullString().Trim(), StringComparison.OrdinalIgnoreCase);
        }
    }
}