...
{
    private sealed class DefaultOrder : IComparer<SyntaxToken>
    {
        ...
        public int Compare(SyntaxToken x, SyntaxToken y) => GetIndex(x.Text) - GetIndex(y.Text);
    ...
    public static Doc PrintSorted(SyntaxTokenList modifiers, PrintingContext context) =>
        PrintWithSortedModifiers(modifiers, context, sortedModifiers => Doc.Group(Doc.Join(" ", sortedModifiers.Select(o => Token.Print(o, context))), " "));

    public static Doc PrintSorterWithoutLeadingTrivia(SyntaxTokenList modifiers, PrintingContext context) =>
        PrintWithSortedModifiers(
            modifiers,
            context,
            sortedModifiers =>
                Doc.Group(
                    ...
                    " ",
                    sortedModifiers.Count > 1 ? Doc.Concat(sortedModifiers.Skip(1).Select(o => Token.PrintWithSuffix(o, " ", context)).ToArray()) : Doc.Null));

    private static Doc PrintWithSortedModifiers(in SyntaxTokenList modifiers, PrintingContext context, Func<IReadOnlyList<SyntaxToken>, Doc> print)
    {
        if (modifiers.Count == 0)
            return Doc.Null;
        ...
        var willReorderModifiers =
            modifiers.Count > 1 && !modifiers.Skip(1).Any(o => o.LeadingTrivia.Any(p => p.IsDirective || p.IsComment())) && !modifiers[0].LeadingTrivia.Any(p => p.IsDirective);
        ...
        if (willReorderModifiers && !sortedModifiers.SequenceEqual(modifiers))
        {
            ...
            var leadingTokenIndex = Array.FindIndex(sortedModifiers, token => token == leadingToken);
            sortedModifiers[leadingTokenIndex] = sortedModifiers[leadingTokenIndex].WithLeadingTrivia(default(SyntaxTriviaList));
