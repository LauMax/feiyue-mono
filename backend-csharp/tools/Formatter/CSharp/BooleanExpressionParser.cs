...
internal abstract class BooleanExpressionParser
{
    ...
    {
        static bool IsPartOfValue(char character) => char.IsLetter(character) || char.IsNumber(character) || character == '_';
    ...
    {
        Dictionary<string, IndexExpression> variables = [];
        ...
        foreach (var token in tokens)
        {
            switch (token.Type)
            {
                case TokenType.Variable:
                    ...
                    {
                        param = Expression.Property(paramDict, typeof(Dictionary<string, bool>).GetProperty("Item")!, Expression.Constant(token.Value));
        ...
        return new() { Function = lambda.Compile(), Parameters = variables.Select(o => o.Key).ToList() };
    }
}
