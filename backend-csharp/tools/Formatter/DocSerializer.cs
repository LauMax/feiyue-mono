...
internal static class DocSerializer
{
    ...
    {
        void AppendIndent() => result.Append(' ', indent * 4);
        void AppendNextIndent() => result.Append(' ', (indent + 1) * 4);
        ...
        else if (doc is StringDoc stringDoc)
        {
            ...
            result.Append("\"" + stringDoc.Value.Replace("\"", "\\\"", StringComparison.Ordinal) + "\"");
            if (stringDoc.IsDirective)
                result.Append(')');
        }
        ...
        {
            AppendIndent();
            result.Append("Doc.HardLine" + (hardLine.Squash ? "IfNoPreviousLine" : string.Empty) + (hardLine.SkipBreakIfFirstInGroup ? "SkipBreakIfFirstInGroup" : string.Empty));
        }
        ...
        else if (doc is ConditionalGroup conditionalGroup)
        {
            ...
            for (var x = 0; x < conditionalGroup.Options.Count; x++)
            {
                Serialize(conditionalGroup.Options[x], result, indent + 1);

                if (x < conditionalGroup.Options.Count - 1)
                    result.AppendLine(",");
            }
        ...
        {
            AppendIndent();
            result.AppendLine(CultureInfo.InvariantCulture, $"Doc.Group{(group.GroupId is not null ? "WithId" : string.Empty)}(");
        ...
        {
            AppendIndent();
            result.AppendLine(CultureInfo.InvariantCulture, $"Doc.{(doc is IndentDoc ? "Indent" : doc.GetType().Name)}(");
        ...
        else if (doc is IndentIfBreak indentIfBreak)
        {
            ...
            result.AppendLine(",");
            AppendNextIndent();
            result.Append(CultureInfo.InvariantCulture, $"\"{indentIfBreak.GroupId}\"").AppendLine();
            AppendIndent();
            result.Append(')');
        }
        else if (doc is IfBreak ifBreak)
        {
            ...
            if (ifBreak.GroupId is not null)
            {
                result.AppendLine(",");
                AppendNextIndent();
        ...
        {
            AppendIndent();
            result.Append(CultureInfo.InvariantCulture, $"Doc.LeadingComment(\"{leadingComment.Comment}\", CommentType.{leadingComment.Type})");
        }
        else if (doc is TrailingComment trailingComment)
        {
            AppendIndent();
            result.Append(CultureInfo.InvariantCulture, $"Doc.TrailingComment(\"{trailingComment.Comment}\", CommentType.{trailingComment.Type})");
        }
        else if (doc is Region region)
        {
            ...
            if (region.IsEnd)
                ...
                result.Append(CultureInfo.InvariantCulture, $"""Doc.BeginRegion("{region.Text}")""");
        }
        else
        {
            throw new("Can't handle " + doc);
        }
    }
}