...
internal static class DocFitter
{
        ...
        StringBuilder output)
    {
        ...
        void Push(Doc doc, PrintMode printMode, Indent indent) => newCommands.Push(new PrintCommand(indent, printMode, doc));
        ...
        for (var x = 0; x < remainingCommands.Count || newCommands.Count > 0; )
        {
            ...
            switch (currentDoc)
            {
                ...
                case StringDoc stringDoc:
                    ...
                    if (stringDoc.Value is null || stringDoc.IsDirective)
                        continue;
                ...
                case Group group:
                {
                    ...
                    var groupContents = groupMode == PrintMode.Break && group is ConditionalGroup conditionalGroup ? conditionalGroup.Options[^1] : group.Contents;
                ...
                {
                    var ifBreakMode = ifBreak.GroupId is not null && groupModeMap.TryGetValue(ifBreak.GroupId, out var groupMode) ? groupMode : currentMode;

                    var contents = ifBreakMode == PrintMode.Break ? ifBreak.BreakContents : ifBreak.FlatContents;
                ...
                case AlwaysFits:
                    break;
                default:
                    throw new("Can't handle " + currentDoc.GetType());
            }
        }
